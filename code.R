...
Metrics_store_ftn <- function(B, p, Sigma_true) {
  ...
  for (b in 1:B) {
    ...
    Ent_mat[b,2] <- Ent_loss(SPCOV, Uns)
    Quad_mat[b,2] <- Quad_loss(SPCOV, Uns)
    MCC_mat[b,2] <- MCC_index(SPCOV, Uns)
    ...
    covrge_diag_mat[b,2] <- {Uns[5,5] >= quantile(sapply(1:5000, function(i) {BGLasso[[i]][5,5]}), 0.025)} & {Uns[5,5] <= quantile(sapply(1:5000, function(i) {BGLasso[[i]][5,5]}), 0.975)}
    covrge_offdiag_mat[b,2] <- {Uns[7,8] >= quantile(sapply(1:5000, function(i) {BGLasso[[i]][7,8]}), 0.025)} & {Uns[7,8] <= quantile(sapply(1:5000, function(i) {BGLasso[[i]][7,8]}), 0.975)}
